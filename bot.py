...
<body>
    <h2>📅 班級互動式課表</h2>
    <div style="font-size: 13px; color: #949ba4; margin-bottom: 15px;">點擊任意課程格子即可彈出詳細資訊（週六開放至第4節）</div>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', async () => {
            if (window.DiscordSDK) {
                const clientId = "1548621269434507325";  // 已帶入你的 Client ID
...
@bot.tree.command(
    name="檢視課表",
    description="[公開] 直接在 Discord 內嵌開啟互動式課表視窗",
)
async def slash_view_schedule(interaction: discord.Interaction):
  ...
  view.add_item(
      discord.ui.Button(
          label="📅 在 Discord 內開啟互動課表",
          style=discord.ButtonStyle.link,
          url=web_url,
      )
  )
  await interaction.response.send_message(
      "這是您的專屬課表（支援 Discord 內部嵌入與點擊互動）：",
      view=view,
      ephemeral=True,
  )
